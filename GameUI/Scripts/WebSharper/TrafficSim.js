(function()
{
 var Global=this,Runtime=this.IntelliFactory.Runtime,List,T,UI,Next,Client,Doc,AttrProxy,Attr,Concurrency,Remoting,AjaxRemotingProvider,Jatek,GameModule,console,Collections,MapModule,Seq,SceneModule,VisualModule;
  ...
  TrafficSim:{
   Client:{
    Main:function()
    {
     ...
     arg201=List.ofArray([input,Doc.Element("button",List.ofArray([Attr.Handler("click",function()
     {
      return function()
      {
       ...
       {
        return Concurrency.Bind(AjaxRemotingProvider.Async("TrafficSim:0",[input.GetValue()]),function(_arg11)
        {
    ...
    Start:function(input,k)
    {
     ...
     arg00=Concurrency.Delay(function()
     {
      return Concurrency.Bind(AjaxRemotingProvider.Async("TrafficSim:0",[input]),function(_arg1)
      {
   ...
   VisualTest:{
    Main:function()
    {
     return GameModule.Create(400,400,function()
     {
      return console?console.log("test"):undefined;
     },MapModule.OfArray(Seq.toArray(List.ofArray([[1,SceneModule.Create(List.ofArray([VisualModule.Create("App_Themes/Standard/BlueHexagon.png",50,50,60,52)]))]]))),1).get_Visual();
    }
   }
  }
 });
 Runtime.OnInit(function()
 {
  ...
  return VisualModule=Runtime.Safe(Jatek.VisualModule);
 });
